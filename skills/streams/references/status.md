...
- **unknown driver** — the branch or a tick branch moved recently but the row's session is not in `ListAgents`. Not a stall: someone is working and the seat cannot see who. Report it first; the human usually knows (a session resumed from a transcript does not register with the others). If the row says the stream is the human's own, this is expected and is not reported.
